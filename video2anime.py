...
def cvt2anime_video(video, output, checkpoint_dir, output_format='MP4V'):
    ...
    with tf.compat.v1.Session(config=tfconfig) as sess:
        ...
        video_out = cv2.VideoWriter(os.path.join(output, vid_name.rsplit('.', 1)[0] + "_result.mp4"), codec, fps, (width, height))

        pbar = tqdm(total=total, ncols=80)
        pbar.set_description(f"Making: {os.path.basename(video).rsplit('.', 1)[0] + '_result.mp4'}")
        while True:
            ret, frame = vid.read()
            if not ret:
                break
        ...
        vid.release()
        video_out.release()
        return os.path.join(output, vid_name.rsplit('.', 1)[0] + "_result.mp4")
